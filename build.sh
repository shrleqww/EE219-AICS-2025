...
build_proj() {
    ...
    verilator --x-assign unique --cc --exe --trace --assert -O3 -Wno-TIMESCALEMOD $VERILATORFLAGS \
        -CFLAGS "-std=c++11 -Wall $INCLUDE_CSRC_FOLDERS $CFLAGS" \
        -LDFLAGS "$LDFLAGS" \
        -o $PROJECT_PATH/$BUILD_FOLDER/$EMU_FILE \
        -Mdir $PROJECT_PATH/$BUILD_FOLDER/emu-compile \
        $INCLUDE_VSRC_FOLDERS $V_TOP_FILE $CSRC_FILES --build -j $(nproc)
