...
#define dataE_baseaddr		0x81000000 // task3_2 new

#define result_1_baseaddr 	0x80900000
#define result_2_baseaddr 	0x80a00000
#define result_3_baseaddr 	0x81100000
...
uint32_t *readtxt_vec72(const char *img){
    uint32_t *data ;
    data = new uint32_t[72];
...
void load_vector72(const char *img){
    uint32_t the_addr ;
    uint32_t the_data ;

    uint32_t *data ;
    data = readtxt_vec72(img);

    the_addr = dataE_baseaddr;
    for(uint32_t i=0; i<72; i++){
        the_data = data[i];
        pmem_write(the_addr, the_data);
        the_addr = the_addr + 1*sizeof(uint32_t);
...
int main(int argc, char **argv) {
	...
	char default_imgpath_task_3[]   = "./build/sw/ramdata_task3_2.txt" ;
	...
	char default_savepath_5[]  = "./build/sw/dumpdata_5.txt" ;

	char *instpath, *imgpath ;
	char *savepath_1, *savepath_2, *savepath_3, *savepath_4, *savepath_5;

	if (argc != 8){
		printf("ERROR: wrong number of arguments\n");
		instpath    = default_instpath ;
		// imgpath     = default_imgpath ;
		imgpath     = (string(instpath).find("task3_2.bin") != string::npos) ? default_imgpath_task_3 : default_imgpath;
		savepath_1  = default_savepath_1 ; 
		savepath_2  = default_savepath_2 ;
		savepath_3  = default_savepath_3 ;
		savepath_4  = default_savepath_4 ;
		savepath_5  = default_savepath_5 ;
	}else{
		instpath   	= argv[1] ;
		imgpath    	= argv[2] ;
		savepath_1  = argv[3] ;
	...
	printf("Using Saving File Path: \n%s\n%s\n%s\n%s\n%s\n",savepath_1, savepath_2, savepath_3, savepath_4, savepath_5);
	...
	if (string(instpath).find("task3_2.bin") != string::npos) {
		load_vector72(imgpath);
		color_printf("g", "Loading task3_2 RAM done !!!\n");
	} else {
		load_ram(imgpath);
		color_printf("g", "Loading Image RAM done !!!\n");
}
	...
	dump_data( result_3_baseaddr, 16*8, savepath_5 );
