...
## Softmax
The softmax (also called softargmax) function is a smooth approximation to the argmax function. The softmax function $σ(x)$ operates on a vector of real-valued scores $x_i$ and normalizes them into a probability distribution

$$p(i) = \sigma(x)_i = \cfrac{e^{x_i}}{\sum_k e^{x_k}}$$

where $p_i \geq 0$ and $\sum_i p_i = 1$ [1].
...
We can use the Q16.16 format to compute the softmax.
In a 32-bit register, the highest bit represents the sign, bits 30–16 represent the integer part, and bits 15–0 represent the fractional part.
Essentially, it is a 32-bit integer. If we interpret the value in the register directly as an int32, then the Q16.16 value is int32 >> 16.

For example, if we want to store a Q16.16 number $x$ in a processor that only supports `int32`, we simply multiply the number by $2^{16}$ ($x$ << 16) to convert it into an integer, which can then be stored in the processor.
When we want to retrieve the actual value from the stored number, we just need to right-shift it by 16 bits.
...
During this Softmax, we do not calculate $e^{x_i}$ directly because it may lead to data overflow. Instead, we perform `exp` operation on the delta between max($x_i$) and each $x_i$. The delta values are clipped to [-8, 0] because $delta = x_i - max(x_i)$ will never be larger than 0, and $e^{-8}$ is small enough to regard it as 0. 
...
exp_sum = sum(exp(DELTA[i]))

if exp_sum <= 0: exp_sum = 1

to prevent data overflow:
  exp_shr[i] = exp(DELTA[i]) >> 2
  exp_sum    = exp_sum >> 2
  if exp_sum <= 0: exp_sum = 1

exp_shr_16 = exp_shr * Q_16
softmax_result = exp_shr_16 // exp_sum
```
...
| original value for Softmax    | 0x81000000             |
| LUT for Softmax exp operation | 0x81000000 + 32        |
| result of Softmax             | 0x81100000             |
...
vredsum.vs vx3, vx2,    vx0,     1  ; vx3[0] = sum( vx0[0], vx2[0], vx2[1], vx2[2]..., vx2[-1] )
vredmax.vs vx3, vx2,    vx0,     1  ; vx3[0] = max( vx0[0], vx2[0], vx2[1], vx2[2]..., vx2[-1] )
...
| ----- | ----------- | ------ | ----- | ------------------------ | ----------------------- |
| LOAD  | `vle32.v`   | 000000 | 110   | vle32.v vd, rs1, vm      | vd = Mem[x[rs1]]        |
| STORE | `vse32.v`   | 000000 | 110   | vse32.v vs3, rs1, vm     | Mem[x[rs1]] = vs3       |
...
| `Arithmetic Inst` | ![RVV_Arithmetic](doc/image/RVV_Arithmetic.png) |
...
1. [M. Dukhan and A. Ablavatski, "Two-Pass Softmax Algorithm," 2020 IEEE International Parallel and Distributed Processing Symposium Workshops (IPDPSW), New Orleans, LA, USA, 2020, pp. 386-395, doi: 10.1109/IPDPSW50202.2020.00074](https://ieeexplore.ieee.org/abstract/document/9150394)
