...
**Notice:** If you use any other HDL languages such as SpinalHDL, Chisel, etc., please make sure that the generated `Verilog` files are compatiable to our testbenches and you should submit the `original files` of other HDL languages as described at the end of this file.
...
**Notes:** In this lab, for simplification, the memory is written row by row. The `mem_addr` is the row index. For example, `mem_addr = 2` if you are sending the results of the third row.
...
## Submission
Please compress all the files in your `vsrc/src` folder into a `zip` file with name `{StudentNumber}_EE219_Lab3.zip`, and submit to Gradescope. If your `Verilog` files are generated by other HDL languages, the original HDL files should be packed into another folder *(You can name this folder whatever you like)*. The file structure should be like
```
...
|-- src
    ...
    |-- systolic_array.v
    `-- ...
|-- additional_folder (for HDL files other than Verilog)
    `-- ...
```
